...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    base_trainer = recursive_find_python_class(
        join(iseek.__path__[0], 'pipelines', 'training', 'trainers'),
        trainer_name,
        'iseek.pipelines.training.trainers'
    )
    if base_trainer is None:
        raise RuntimeError(f'Trainer {trainer_name} not found in '
                           f'iseek.pipelines.training.trainers')
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if plans_identifier == 'nnUNetPlans':
        rprint(f"INFO: You are using the nnU-Net plans")
...
# ############################################################
# ############ nnUNet based training framework. ##############
# ############################################################
